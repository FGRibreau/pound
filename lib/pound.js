...
Pound.prototype._renderScriptTags = function(req, res){
  var inline = "";

  if(res._responseObs){
    for(var ob in res._responseObs){
      inline += this.client.js.toGlobals(ob);
    }
  }

  if(res._responseFns){
    for(var fn in res._responseFns){
      inline += this.client.js.executableFrom(fn);
    }
  }

  return function(){
    return this.client.js.renderTags.apply(this.client.js, arguments) + (inline ? '<script type="text/javascript">{'+inline+'}</script>' :'');
  }.bind(this);
};

Pound.prototype._renderStyleTags = function(){
  return function(){
    return this.client.css.renderTags.apply(this.client.css, arguments);
  }.bind(this);
};

Pound.prototype._setDynamicHelpers =function(app){

  if(_.isFunction(app.dynamicHelpers)) {
    // Support for Express 2
    app.dynamicHelpers({
        renderStyleTags: this._renderStyleTags,
        renderScriptTags: this._renderScriptTags
    });

    return;
  }

  if(_.isFunction(app.locals)) {
    // Support for Express 3
    app.use(function middleware(req, res, next) {
      process.nextTick(function () {
        res.locals.renderStyleTags = this._renderStyleTags(req, res);
        res.locals.renderScriptTags = this._renderScriptTags(req, res);
        next();
      }.bind(this));
    }.bind(this));
  ...
  throw new Error("[Pound] Unsupported version of Express");
};
