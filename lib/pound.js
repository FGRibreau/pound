...
var debug = require('debug')('pound');

function iffb(b, r) {
  return typeof (b) === "boolean" ? b : r;
}
...
function Pound(options) {
  ...
  debug('isProduction=%s', this.isProduction);
  ...
  this.resolve = {
    css: function (filename) {
      return this.publicDir + '/css/' + filename + '.css';
    },
    js: function (filename) {
      return this.publicDir + '/js/' + filename + '.js';
    }
  };
}
...
Pound.prototype._addAssetGroup = function (bundleUp, assetType, type, assetGroup) {
  if (assetGroup.indexOf(this._publicNS) !== -1) {
    return;
  }
...
Pound.prototype._addResource = function (bundleUp, type, assetGroup, resource) {
  ...
  if (resource.substring(0, 7) === 'http://' || resource.substring(0, 8) === 'https://') { // url v2
    //console.log(assetGroup, 'addUrl', resource);
  ...
  if (resource[0] === '$') { // static/${name}.type
    ...
    if (this.userDefinedTypes[type]) {
      ...
      return specialType.handler(bundleUp, assetGroup, resource, this.resolve[fileType] ? this.resolve[fileType].bind(this, filename) : function () {
        throw new Error('Undefined resolve() for "' + fileType + '"');
      });
    }
